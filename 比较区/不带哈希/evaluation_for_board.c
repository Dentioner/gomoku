...
long int special_evaluation(int step_count, bool my_turn, int raw, int column, int temp_board[][15])//新加
//step_count的作用是，确认这盘是黑子还是白子
//my_turn 的作用是，确认这盘是我方还是敌方
//raw和column是传递坐标，看看到底是哪个位置需要评估
//将temp_board传进来，是为了将棋子做一些修改，让某些函数无法识别这些棋子
{
	//先确认这把是白子下还是黑子下
	int chess;
	int opponent_chess;
	int special_chess = 20;//这个特殊的数字代表一个特殊的棋子，它将覆盖被检索过的棋子
		...
		&& (board[raw][column - 4] == chess))
	{
		...
		temp_board[raw][column - 1] = special_chess;
		temp_board[raw][column - 2] = special_chess;
		temp_board[raw][column - 3] = special_chess;
		temp_board[raw][column - 4] = special_chess;
	}
	//●●●?●型
	if ((board[raw][column - 1] == chess)
		&& (board[raw][column - 2] == chess)
	...
	{
		value += Consecutive_Five;
		temp_board[raw - 1][column] = special_chess;
		temp_board[raw - 2][column] = special_chess;
		temp_board[raw - 3][column] = special_chess;
		temp_board[raw - 4][column] = special_chess;
	}
	...
	{
		value += Consecutive_Five;
		temp_board[raw + 1][column - 1] = special_chess;
		temp_board[raw + 2][column - 2] = special_chess;
		temp_board[raw + 3][column - 3] = special_chess;
		temp_board[raw + 4][column - 4] = special_chess;
	}
